...
from .interfaces import ISessionCommand, CommandResult, IMoodleClient, IContentProcessor, IEventPublisher
...
class BaseCommand(ISessionCommand):
    ...
    async def execute(self, session_data: Dict[str, Any]) -> CommandResult:
        ...
        try:
            ...
            logger.debug(f"Command {self.__class__.__name__} executed in {self.execution_time:.3f}s")
            return result
    ...
    async def _undo_impl(self, session_data: Dict[str, Any]) -> CommandResult:
        ...
        return CommandResult(
            success=False,
            message=f"Undo not supported for {self.__class__.__name__}"
        )
...
class CreateCourseCommand(BaseCommand):
    ...
    def __init__(self, context: CommandContext, moodle_client: IMoodleClient, course_name: str, course_description: str = ""):
    ...
    async def _undo_impl(self, session_data: Dict[str, Any]) -> CommandResult:
        ...
        logger.warning(f"Undo requested for course creation (ID: {self.created_course_id}). Manual cleanup may be required.")
...
class CreateCourseStructureCommand(BaseCommand):
    ...
    def __init__(self, context: CommandContext, moodle_client: IMoodleClient, course_id: int, sections_data: List[Dict[str, Any]]):
        super().__init__(context)
        self.moodle_client = moodle_client
    ...
    async def _execute_impl(self, session_data: Dict[str, Any]) -> CommandResult:
        ...
        try:
            ...
            if "sections" in result:
                self.created_sections = [s.get("id") for s in result["sections"] if s.get("id")]
            ...
            logger.error(f"Failed to create course structure for course {self.course_id}: {e}")
            raise
    
    async def _undo_impl(self, session_data: Dict[str, Any]) -> CommandResult:
        """Undo course structure creation (limited support)"""
        logger.warning(f"Undo requested for course structure creation (Course ID: {self.course_id}). Manual cleanup may be required.")
...
class ProcessContentCommand(BaseCommand):
    ...
    def __init__(self, context: CommandContext, content_processor: IContentProcessor, content: str):
        super().__init__(context)
...
class ValidateCourseCommand(BaseCommand):
    ...
    def __init__(self, context: CommandContext, moodle_client: IMoodleClient, course_id: int, expected_sections: int):
        super().__init__(context)
        self.moodle_client = moodle_client
        self.course_id = course_id
...
class CommandExecutor:
    ...
    async def execute_command(self, command: BaseCommand, session_data: Dict[str, Any]) -> CommandResult:
        """
    ...
    async def execute_commands(self, commands: List[BaseCommand], session_data: Dict[str, Any]) -> List[CommandResult]:
        """
        ...
        for command in commands:
            try:
                ...
                if not result.success and getattr(command, 'critical', True):
                    logger.warning(f"Critical command failed, stopping execution: {command.__class__.__name__}")
                    break
                ...
                results.append(CommandResult(
                    success=False,
                    message=f"Command execution failed: {str(e)}",
                    data={"command_type": command.__class__.__name__, "error": str(e)}
                ))
                break
    ...
    async def undo_last_command(self, session_data: Dict[str, Any]) -> CommandResult:
        ...
        if last_command.status != CommandStatus.COMPLETED:
            return CommandResult(success=False, message="Last command was not successfully completed")
        
        result = await last_command.undo(session_data)
        return result
    
    async def undo_commands_for_session(self, session_id: str, session_data: Dict[str, Any]) -> List[CommandResult]:
        """Undo all commands for a specific session"""
        session_commands = [cmd for cmd in self._command_history if cmd.context.session_id == session_id]
    ...
    def get_command_history(self, session_id: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        completed = sum(1 for cmd in self._command_history if cmd.status == CommandStatus.COMPLETED)
        failed = sum(1 for cmd in self._command_history if cmd.status == CommandStatus.FAILED)
        
        execution_times = [cmd.execution_time for cmd in self._command_history if cmd.execution_time]
