...
class ContentPreprocessor:
    ...
    def __init__(self):
        ...
        self.HIGH_UNICODE_PATTERN = re.compile(
            r"[^\x00-\x7F\u00A0-\u017F\u0100-\u024F]"
        )

    def sanitize_sections_data(
        ...
        """
        logger.info(
            f"Preprocessing {len(sections_data)} sections for API compatibility"
        )
        ...
        for i, section in enumerate(sections_data):
            ...
            if total_payload_size > self.MAX_TOTAL_PAYLOAD_SIZE:
                logger.warning(
                    f"Payload size limit reached at section {i}. Truncating remaining sections."
                )
                break

            sanitized_sections.append(sanitized_section)
            logger.debug(
                f"Section {i}: {section_size} bytes, total: {total_payload_size} bytes"
            )

        logger.info(
            f"Preprocessed {len(sanitized_sections)} sections, total size: {total_payload_size} bytes"
        )
    ...
    def _sanitize_section(
        ...
        sanitized = {
            "name": self._sanitize_text(
                section.get("name", f"Section {section_index + 1}")
            ),
        ...
        for j, activity in enumerate(section.get("activities", [])):
            ...
            if activity_size > self.MAX_SINGLE_PARAM_SIZE:
                logger.warning(
                    f"Activity {j} in section {section_index} too large ({activity_size} bytes). Truncating."
                )
    ...
    def _sanitize_activity(
        self, activity: Dict[str, Any], section_idx: int, activity_idx: int
    ) -> Dict[str, Any]:
        ...
        sanitized = {
            ...
            "name": self._sanitize_text(
                activity.get("name", f"Activity {activity_idx + 1}")
            ),
            "content": self._sanitize_content(
                activity.get("content", ""), activity_type
            ),
    ...
    def _sanitize_html(self, html_content: str) -> str:
        ...
        html_content = re.sub(
            r'(on\w+)=["\'][^"\']*["\']', "", html_content, flags=re.IGNORECASE
        )
    ...
    def _sanitize_content(self, content: str, activity_type: str) -> str:
        ...
        if len(content) > self.MAX_SINGLE_PARAM_SIZE:
            logger.warning(
                f"Content too large ({len(content)} bytes), truncating to {self.MAX_SINGLE_PARAM_SIZE}"
            )
            content = (
                content[: self.MAX_SINGLE_PARAM_SIZE - 100]
    ...
    def _truncate_activity(self, activity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(content) > self.MAX_SINGLE_PARAM_SIZE:
            ...
            truncate_size = (
                self.MAX_SINGLE_PARAM_SIZE - 200
            ...
            logger.info(
                f"Truncated activity content from {len(content)} to {len(activity['content'])} bytes"
            )
    ...
    def get_preprocessing_stats(
        self, original_data: List[Dict], sanitized_data: List[Dict]
    ) -> Dict[str, Any]:
