...
class UserPreferences:
    ...
    default_section_names: List[str] = field(
        default_factory=lambda: ["Overview", "Content", "Activities", "Resources"]
    )
    content_chunking_strategy: str = "intelligent"  # simple, intelligent, progressive
    activity_types_preference: List[str] = field(
        default_factory=lambda: ["page", "label", "file"]
    )
...
class ContextAwareProcessor:
    ...
    def _cleanup_old_contexts(self):
        ...
        sorted_contexts = sorted(
            self.contexts.items(), key=lambda x: x[1].last_activity
        )
    ...
    def add_conversation_turn(
        self, session_id: str, user_input: str, processing_outcome: str = "pending"
    ) -> ConversationTurn:
    ...
    def _determine_context_type(
        self, context: ConversationContext, intent: str, sentiment: str
    ) -> str:
        ...
        if (
            last_turn.processing_outcome == "failure"
            or sentiment == "negative"
            or intent == "troubleshoot"
        ):
            return "error_recovery"

        # Follow-up context
        if intent == last_turn.extracted_intent or any(
            topic in context.accumulated_topics
            for topic in getattr(last_turn, "content_topics", [])
        ):
            return "follow_up"
    ...
    def get_contextual_suggestions(self, session_id: str) -> Dict[str, Any]:
        ...
        recent_failures = [
            turn
            for turn in list(context.conversation_history)[-5:]
        ...
        if context.accumulated_topics:
            ...
            suggestions.append(
                f"Consider organizing content around {most_common_topic}"
            )
    ...
    def get_adaptive_processing_strategy(
        ...
        recent_successes = [
            turn
            for turn in list(context.conversation_history)[-3:]
    ...
    def get_global_statistics(self) -> Dict[str, Any]:
        ...
        total_processes = (
            self.global_statistics["successful_processes"]
        ...
        return {
            ...
            "success_rate": (
                self.global_statistics["successful_processes"]
                / max(total_processes, 1)
                * 100
            ),
            "most_common_topics": dict(
                sorted(
                    self.global_statistics["most_common_topics"].items(),
                    key=lambda x: x[1],
                    reverse=True,
                )[:5]
            ),
...
if __name__ == "__main__":
    ...
    # User has an issue
    processor.add_conversation_turn(
        session_id, "The course creation failed, what went wrong?", "failure"
    )
