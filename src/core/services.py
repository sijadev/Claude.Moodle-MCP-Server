...
class CourseCreationService(ICourseCreationService):
    ...
    async def create_course_from_content(
        self, content: str, course_name: str, options: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            complexity_analysis = (
                await self.content_processor.analyze_content_complexity(content)
            )
            ...
            session_data = {
                ...
                "state": "created",
                "strategy": complexity_analysis.get(
            ...
            # Start processing
            await publish_processing_started(
                self.event_publisher,
                session_id,
            ...
            results = await self.command_executor.execute_commands(
            ...
            if all(result.success for result in results):
                ...
                if course_id:
                    ...
                    course_structure_data = await self._build_course_structure_data(
                        session_id
                    )
                    if course_structure_data:
                        ...
                        structure_result = await self.command_executor.execute_command(
                            structure_command, session_data
                        )
            ...
            await publish_session_failed(
                self.event_publisher, session_id, {"error": str(e)}
            )
    ...
    async def continue_course_creation(
    ...
    async def validate_course(
        self, session_id: str, course_id: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            expected_sections = len(
                session_data.get("course_structure", {}).get("sections", [])
            )
            ...
            result = await self.command_executor.execute_command(
                validation_command, session_data
            )
    ...
    async def _build_course_structure_data(
        self, session_id: str
        ...
        try:
            ...
            logger.error(
                f"Failed to build course structure for session {session_id}: {e}"
            )
            return None
    ...
    async def _handle_creation_failure(self, session_id: str, results: List) -> None:
        ...
        await publish_session_failed(
            self.event_publisher, session_id, {"errors": error_messages}
        )
...
class AnalyticsService(IAnalyticsService):
    ...
    def __init__(
        self, session_repository: ISessionRepository, event_publisher: IEventPublisher
    ):
        self.session_repository = session_repository
        self.event_publisher = event_publisher
    ...
    async def record_session_metrics(
        ...
        try:
            ...
            if session_data:
                ...
                await self.session_repository.update_session_state(
                    session_id, session_data["state"], {"metrics": current_metrics}
                )
    ...
    async def get_processing_analytics(self, detailed: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if detailed:
                analytics["detailed_metrics"] = await self._get_detailed_metrics(
                    active_sessions
                )

            # Cache results
            self._metrics_cache = analytics
            self._cache_expires = datetime.now().replace(
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_rate = self._calculate_error_rate(
                repo_stats.get("sessions_by_state", {})
            )
    ...
    def _analyze_timeline(self, sessions: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        now = datetime.now()
        timeline = {"last_hour": 0, "last_24_hours": 0, "last_week": 0}
    ...
    async def cleanup_expired_sessions(self) -> Dict[str, Any]:
        ...
        try:
            if hasattr(self.session_repository, "cleanup_expired_sessions"):
                ...
                return {"sessions_deleted": deleted_count, "cleanup_completed": True}
            else:
                return {
