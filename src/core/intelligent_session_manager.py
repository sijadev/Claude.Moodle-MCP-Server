...
from src.core.adaptive_content_processor import AdaptiveContentProcessor, ProcessingSession, SessionState
...
class IntelligentSessionManager:
    ...
    def __init__(self, moodle_client: Optional[EnhancedMoodleClient] = None, 
        ...
        try:
            with sqlite3.connect(self.db_config.db_path) as conn:
                ...
                for row in cursor:
                    ...
                    session_data['created_sections'] = json.loads(session_data['created_sections'] or '[]')
                    session_data['needs_continuation'] = bool(session_data['needs_continuation'])
                ...
                logger.info(f"Loaded {len(self.content_processor.active_sessions)} active sessions from database")
    ...
    def _save_session_to_db(self, session: ProcessingSession):
        ...
        try:
            with sqlite3.connect(self.db_config.db_path) as conn:
                ...
                session_data['created_sections'] = json.dumps(session_data['created_sections'])
                session_data['needs_continuation'] = int(session_data['needs_continuation'])
                ...
                """, (
                    session_data['session_id'], session_data['content_hash'], session_data['original_content'],
                    session_data['course_name'], session_data['strategy'], session_data['state'],
                    session_data['total_chunks'], session_data['processed_chunks'], session_data['current_chunk_index'],
                    session_data['course_id'], session_data['created_sections'], session_data['error_count'],
                    session_data['last_error'], session_data['retry_attempts'], session_data['needs_continuation'],
                    session_data['continuation_prompt'], session_data['created_at'], session_data['updated_at'],
    ...
    async def _periodic_backup(self):
        ...
        while True:
            try:
                ...
                # Create backup
                backup_path = f"{self.db_config.db_path}.backup.{int(datetime.now().timestamp())}"
    ...
    async def create_intelligent_course_session(self, content: str, course_name: str = "", 
                                               continue_previous: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if analysis['recommended_strategy'].value == 'single_pass':
                ...
                success, result = await self.content_processor.process_content_chunk(session_id, 0)
                
                if success:
                    # Create actual Moodle course if client is available
                    if self.moodle_client:
                        course_result = await self._create_moodle_course(session, result['course_structure'])
                ...
                return {
                    ...
                    "user_friendly_message": self._generate_user_start_message(analysis)
                }
    ...
    async def continue_session_processing(self, session_id: str, 
        ...
        try:
            ...
            if success:
                ...
                if self.moodle_client and 'course_structure' in result:
                    course_result = await self._update_moodle_course(session, result['course_structure'])
                    result.update(course_result)
                ...
                # Record metrics
                await self._record_session_metrics(session_id, len(additional_content), True, "")
                ...
                await self._record_session_metrics(session_id, len(additional_content), False, 
            ...
            await self._record_session_metrics(session_id, len(additional_content), False, str(e))
    ...
    async def _create_moodle_course(self, session: ProcessingSession, 
        ...
        try:
            ...
            for section in course_structure.sections:
                try:
                    ...
                    logger.error(f"Error creating section '{section.name}': {section_error}")
                    # Continue with other sections
            
            session.created_sections.extend(created_sections)
            
            # Validate course creation
            validation_result = await self._validate_course_creation(session, course_structure)
    ...
    async def _update_moodle_course(self, session: ProcessingSession, 
                                  course_structure: CourseStructure) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "moodle_integration": "update_failed",
                "error": str(e)
            }
    
    async def _validate_course_creation(self, session: ProcessingSession, 
                                      expected_structure: CourseStructure) -> Dict[str, Any]:
        ...
        try:
            ...
            course_contents = await self.moodle_client.get_course_contents(session.course_id)
            ...
            if actual_sections != expected_sections:
                validation_errors.append(f"Section count mismatch: expected {expected_sections}, got {actual_sections}")
            ...
            with sqlite3.connect(self.db_config.db_path) as conn:
                ...
                """, (
                    session.session_id, session.course_id, expected_sections, actual_sections,
                    validation_status, json.dumps(validation_errors), datetime.now().isoformat()
                ))
                conn.commit()
    ...
    async def _record_session_metrics(self, session_id: str, content_size: int, 
                                    success: bool, error_message: str = ""):
        """Record session processing metrics"""
        try:
            session = self.content_processor.active_sessions.get(session_id)
            processing_time = (datetime.now() - session.updated_at).total_seconds() if session else 0
            ...
            with sqlite3.connect(self.db_config.db_path) as conn:
                ...
                """, (
                    session_id, content_size, processing_time, success,
                    error_message, session.strategy.value if session else "unknown",
        ...
        try:
            with sqlite3.connect(self.db_config.db_path) as conn:
                ...
                return {
                    ...
                    "recent_processing": dict(processing_metrics) if processing_metrics else {},
