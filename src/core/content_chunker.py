...
class ContentChunker:
    ...
    def chunk_course_structure(self, course_structure: CourseStructure) -> List[CourseStructure]:
        """
        ...
        for section in course_structure.sections:
            ...
            if len(section.items) > self.max_section_size:
                logger.info(f"Chunking large section '{section.name}' with {len(section.items)} items")
                ...
                for item in section.items:
                    if len(item.content) > self.max_content_length:
                        logger.info(f"Chunking large content item '{item.title}' ({len(item.content)} chars)")
                        
                        # Split large content into smaller pieces
                        content_chunks = self._split_content(item.content, self.max_content_length)
                        ...
                        for j, content_chunk in enumerate(content_chunks):
                            chunked_item = type(item)(
                                ...
                                topic=item.topic,
                                description=f"{item.description or ''} - Part {j+1} of {len(content_chunks)}" if item.description else f"Part {j+1} of {len(content_chunks)}"
                            )
