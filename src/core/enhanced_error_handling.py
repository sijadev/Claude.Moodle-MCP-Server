...
class EnhancedError(BaseModel):
    ...
    def to_claude_response(self) -> str:
        ...
        if self.technical_details and self.severity in [ErrorSeverity.HIGH, ErrorSeverity.CRITICAL]:
...
class ErrorHandlerMixin:
    ...
    def _add_contextual_suggestions(self, error: EnhancedError, exception: Optional[Exception] = None):
        ...
        if error.category == ErrorCategory.CONFIGURATION:
            ...
            error.user_friendly_message = (
                "Can't connect to your Moodle server. Make sure it's running and accessible."
            )
            ...
            error.user_friendly_message = (
                "The input data doesn't meet the requirements. Please check the format and try again."
            )
            ...
            error.user_friendly_message = (
                "The operation is taking longer than expected. Try with smaller content or check server performance."
            )
            ...
            error.user_friendly_message = (
                "There's an issue with the MoodleClaude plugin. Make sure it's properly installed and configured."
            )
        ...
        if exception:
            ...
            if "Connection" in exception_name or "Timeout" in exception_name:
                error.suggestions.insert(0, "Check network connectivity and server status")
                error.recovery_actions.insert(0, "Verify Moodle server is running and accessible")
                ...
                error.recovery_actions.insert(0, "Verify user has required permissions in Moodle")
            
            elif "NotFound" in exception_name:
                error.suggestions.insert(0, "Check if the requested resource exists")
                error.recovery_actions.insert(0, "Verify the resource ID or path is correct")
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_errors": len([e for e in self.error_history if 
                                (datetime.now() - e.context.timestamp).seconds < 3600]),
            "by_category": by_category,
            "last_error": self.error_history[-1].error_id if self.error_history else None
        }
...
def create_configuration_error(handler: ErrorHandlerMixin, message: str, 
                             missing_config: str = None) -> EnhancedError:
...
def create_moodle_api_error(handler: ErrorHandlerMixin, message: str, 
                           api_function: str = None, exception: Exception = None) -> EnhancedError:
...
def create_validation_error(handler: ErrorHandlerMixin, message: str, 
                           invalid_field: str = None) -> EnhancedError:
...
if __name__ == "__main__":
    ...
    config_error = create_configuration_error(
        handler, 
        "Missing MOODLE_TOKEN environment variable", 
        "MOODLE_TOKEN"
    )
