...
class AdaptiveContentProcessor:
    ...
    async def analyze_content_complexity(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            topic_items = [
                item for item in parsed_content.items if item.type == "topic"
            ]

            analysis["code_blocks"] = len(code_items)
            analysis["topics"] = len(topic_items)
            analysis["estimated_sections"] = max(
                1, (len(parsed_content.items) + 4) // 5
            )  # ~5 items per section
            ...
            analysis["complexity_score"] = (
                length_factor + structure_factor + code_factor + topic_factor
            ) / 4

            # Determine strategy based on analysis
            if (
                analysis["content_length"] <= self.content_limits.max_char_length
                ...
                analysis["estimated_chunks"] = max(
                    2, analysis["estimated_sections"] // 3
                )
            ...
            analysis["processing_time_estimate"] = int(
                base_time * complexity_multiplier * chunk_multiplier
            )

            logger.info(
                f"Content analysis: {analysis['content_length']} chars, "
    ...
    def _analyze_content_complexity_sync(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            structure_factor = min(
                (analysis["estimated_sections"] + analysis["code_blocks"]) / 20, 1.0
            )
            ...
            logger.info(
                f"Sync content analysis: {analysis['content_length']} chars, "
                f"complexity: {analysis['complexity_score']:.2f}, "
                f"strategy: {analysis['recommended_strategy'].value}"
            )
    ...
    def create_session(self, content: str, course_name: str = "") -> str:
            ...
            if loop.is_running():
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run, self.analyze_content_complexity(content)
                    )
        ...
        session = ProcessingSession(
            ...
            or f"Course from Chat {datetime.now().strftime('%Y-%m-%d %H:%M')}",
        )

        self.active_sessions[session_id] = session

        logger.info(
            f"Created session {session_id} with strategy {session.strategy.value}"
        )
        return session_id
    ...
    async def process_content_chunk(
        self, session_id: str, chunk_index: int = 0, continue_previous: bool = False
        ...
        try:
            ...
            logger.error(
                f"Error processing chunk {chunk_index} in session {session_id}: {e}"
            )
    ...
    async def _get_content_chunk(
        self, session: ProcessingSession, chunk_index: int
    ) -> str:
    ...
    def _create_intelligent_chunks(self, parsed_content: ChatContent) -> List[str]:
        ...
        max_chunk_length = int(
            self.content_limits.max_char_length * 0.8
        )  # Safety margin
    ...
    def _create_course_structure(
        self, parsed_content: ChatContent, session: ProcessingSession
    ) -> CourseStructure:
        """Create course structure from parsed content"""
        sections = []
    ...
    async def _generate_intelligent_response(
        ...
        if session.needs_continuation:
            ...
            response["final_summary"] = {
                ...
                "total_items": sum(
                    len(section.items) for section in course_structure.sections
                ),
                "processing_time": (
    ...
    async def _handle_retry_logic(
        self, session: ProcessingSession, chunk_index: int, error: str
    ) -> Tuple[bool, Dict[str, Any]]:
    ...
    def _update_success_metrics(self, content_size: int, success: bool):
        ...
        self.success_metrics["average_content_size"] = (
            current_avg * (total_requests - 1) + content_size
        ) / total_requests
