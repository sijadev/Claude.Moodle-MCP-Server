...
from .interfaces import (
    IMoodleClient, IContentProcessor, ISessionRepository, IEventPublisher,
    ICourseCreationService, IAnalyticsService, IConfiguration
)

# Import implementations
from .event_system import EventPublisher, LoggingObserver, MetricsObserver
from .repositories import SQLiteSessionRepository, InMemorySessionRepository, CachedSessionRepository
from .services import CourseCreationService, AnalyticsService, SessionCoordinatorService
...
def configure_services(container: ServiceContainer, config_options: Dict[str, Any] = None) -> ServiceContainer:
    """
    ...
    if config_options.get("enable_metrics_observer", True):
        ...
        container.register_instance(MetricsObserver, metrics_observer)  # For accessing metrics
    ...
    try:
        moodle_client = EnhancedMoodleClient(
            ...
            plugin_token=dual_config.get_plugin_token() if dual_config.is_dual_token_mode() else None
        )
    ...
    container.register(ICourseCreationService, CourseCreationService, ServiceLifetime.SINGLETON)
    container.register(IAnalyticsService, AnalyticsService, ServiceLifetime.SINGLETON)
    
    # 7. Coordination Services
    container.register(SessionCoordinatorService, SessionCoordinatorService, ServiceLifetime.SINGLETON)
    
    logger.info("Service configuration completed successfully")
    return container


def create_configured_container(config_options: Dict[str, Any] = None) -> ServiceContainer:
    """
