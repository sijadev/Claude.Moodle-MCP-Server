...
import asyncio
import inspect
from typing import Dict, Type, Any, Optional, TypeVar, Callable, Union
import logging
...
class ServiceContainer(IServiceContainer):
    ...
    def register(
        ...
        with self._lock:
            if inspect.isclass(implementation):
                ...
                descriptor = ServiceDescriptor(interface_type, None, lifetime, implementation)
            else:
                # Instance-based registration (always singleton)
                descriptor = ServiceDescriptor(interface_type, implementation, ServiceLifetime.SINGLETON)
                descriptor.instance = implementation
            
            self._services[interface_type] = descriptor
            logger.debug(f"Registered service: {interface_type.__name__} -> {implementation} ({lifetime})")
        
        return self
    
    def register_singleton(self, interface_type: Type[T], implementation: Union[Type[T], Callable[[], T]]) -> 'ServiceContainer':
        """Register as singleton (default)"""
        return self.register(interface_type, implementation, ServiceLifetime.SINGLETON)
    
    def register_transient(self, interface_type: Type[T], implementation: Union[Type[T], Callable[[], T]]) -> 'ServiceContainer':
        """Register as transient (new instance each time)"""
        return self.register(interface_type, implementation, ServiceLifetime.TRANSIENT)
    
    def register_scoped(self, interface_type: Type[T], implementation: Union[Type[T], Callable[[], T]]) -> 'ServiceContainer':
        """Register as scoped (one instance per scope)"""
        return self.register(interface_type, implementation, ServiceLifetime.SCOPED)
    
    def register_instance(self, interface_type: Type[T], instance: T) -> 'ServiceContainer':
        """Register a specific instance"""
        return self.register(interface_type, instance, ServiceLifetime.SINGLETON)
    
    def register_factory(self, interface_type: Type[T], factory: Callable[[], T], lifetime: str = ServiceLifetime.SINGLETON) -> 'ServiceContainer':
    ...
    async def resolve_async(self, interface_type: Type[T], scope_id: Optional[str] = None) -> T:
    ...
    def _create_instance(self, descriptor: ServiceDescriptor) -> Any:
        ...
        try:
            if descriptor.factory:
                ...
                raise ValueError(f"No factory or implementation for {descriptor.interface_type.__name__}")
        
        except Exception as e:
            logger.error(f"Error creating instance for {descriptor.interface_type.__name__}: {e}")
            raise
        ...
        for param in parameters:
            if param.annotation != inspect.Parameter.empty:
                ...
                if self.is_registered(param.annotation):
                    ...
                    logger.warning(f"Cannot resolve dependency {param.annotation} for {implementation_type.__name__}")
