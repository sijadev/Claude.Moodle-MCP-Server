    ...
    def create_snapshot(self, snapshot_name: str, include_cache: bool = False) -> bool:
        ...
        try:
            snapshot_dir = (
                self.storage_dir
                / "snapshots"
                / f"{snapshot_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            if results_src.exists():
                shutil.copytree(
                    results_src, snapshot_dir / "results", dirs_exist_ok=True
                )
            ...
            if benchmarks_src.exists():
                shutil.copytree(
                    benchmarks_src, snapshot_dir / "benchmarks", dirs_exist_ok=True
                )
            ...
            if include_cache:
                ...
                if cache_src.exists():
                    shutil.copytree(
                        cache_src, snapshot_dir / "cache", dirs_exist_ok=True
                    )
    ...
    def get_test_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                stats["success_rate"] = (
                    (stats["passed_tests"] / stats["total_tests"] * 100)
                ...
                cursor = conn.execute(
                    "SELECT AVG(duration) FROM test_results WHERE duration > 0"
                )
    ...
    def cleanup_old_data(self, days: int = 30) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute(
                    "DELETE FROM benchmarks WHERE timestamp < ?", (cutoff_date,)
                )
            ...
            self.logger.info(
                f"Cleaned up {deleted_tests} test results and {deleted_benchmarks} benchmarks"
            )
            return True
