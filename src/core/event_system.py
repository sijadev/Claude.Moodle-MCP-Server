...
class EventPublisher(IEventPublisher):
    ...
    def __init__(self):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=4, thread_name_prefix="event_publisher"
        )

    def subscribe(
        self,
        observer: ISessionObserver,
        ...
        with self._lock:
            if event_types is None:
                ...
                logger.debug(
                    f"Observer {observer.__class__.__name__} subscribed to events: {[e.value for e in event_types]}"
                )
    ...
    async def publish(self, event: SessionEvent) -> None:
        ...
        if tasks:
            ...
            logger.debug(
                f"Published event {event.event_type.value} to {len(observers_to_notify)} observers"
            )

    async def _notify_observer_safe(
        self, observer: ISessionObserver, event: SessionEvent
    ) -> None:
        ...
        with self._lock:
            ...
            if event.event_type == SessionEventType.SESSION_CREATED:
                ...
                if "processing_time" in event.data:
                    self._metrics["processing_times"].append(
...
async def publish_session_created(
    publisher: IEventPublisher, session_id: str, data: Dict[str, Any]
) -> None:
...
async def publish_chunk_processed(
    publisher: IEventPublisher, session_id: str, data: Dict[str, Any]
) -> None:
...
async def publish_course_created(
    publisher: IEventPublisher, session_id: str, data: Dict[str, Any]
) -> None:
...
async def publish_session_failed(
    publisher: IEventPublisher, session_id: str, data: Dict[str, Any]
) -> None:
