...
class MoodleClient:
    ...
    async def _enroll_user_in_course(self, course_id: int):
        ...
        try:
            ...
            try:
                user_courses = await self._call_api(
                ...
                if course_id in enrolled_course_ids:
                    logger.info(
                        f"User {username} already enrolled in course {course_id}"
                    )
                    return
            except Exception as e:
            ...
            for method in enrol_methods:
                if (
                    ...
                    try:
                        ...
                        logger.info(
                            f"Successfully enrolled user {username} in course {course_id} via {method.get('type')}"
                        )
                        return
                    except Exception as e:
                        logger.debug(
                            f"Self-enrollment failed for method {method.get('type')}: {e}"
                        )
                        continue

            logger.warning(
                f"No suitable enrollment methods found for course {course_id}"
            )
        ...
        try:
            ...
            for cat in categories:
                if cat.get("name") == "MoodleClaude Courses":
                    logger.info(
                        f"Found existing MoodleClaude category: {cat.get('id')}"
                    )
                    return cat.get("id")
        ...
        try:
            ...
            for section in sections:
                ...
                if section_num > 0:
                    ...
                    logger.info(
                        f"Updated existing section {section_num} with name '{name}'"
                    )
                    return section_num
    ...
    async def update_section_content(
        ...
        logger.info(
            f"   ℹ️ Section editing not available due to Moodle API/permission limitations"
        )
        logger.info(
            f"   💡 Content will be organized in default sections (Topic 1, Topic 2, etc.)"
        )
    ...
    async def edit_section(
        self, course_id: int, section_id: int, name: str, summary: str = ""
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Section {section_id} edited successfully in course {course_id}"
            )
            return True
        except Exception as e:
    ...
    async def create_page_activity(
        ...
        """
        logger.info(
            f"📄 Attempting to create page activity '{name}' for course {course_id}, section {section_id}"
        )
...
"""
            ...
            success = await self._update_section_summary(
                course_id, section_id, name, section_content
            )
    ...
    async def _update_section_summary(
        self, course_id: int, section_id: int, name: str, content: str
    ) -> bool:
        """
        ...
        logger.info(
            f"   ℹ️ Content storage not available due to Moodle API/permission limitations"
        )
        logger.info(
            f"   💡 Course structure created but content must be added manually in Moodle"
        )
    ...
    async def create_label_activity(
        ...
        """
        logger.info(
            f"🏷️ Attempting to create label activity '{name}' for course {course_id}, section {section_id}"
        )
...
"""
            ...
            success = await self._update_section_summary(
                course_id, section_id, name, label_content
            )
    ...
    async def create_file_activity(
        ...
        """
        logger.info(
            f"📄 Attempting to create file activity '{name}' ({filename}) for course {course_id}, section {section_id}"
        )
...
"""
            ...
            success = await self._update_section_summary(
                course_id, section_id, name, file_content
            )
