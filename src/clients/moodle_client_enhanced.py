...
class EnhancedMoodleClient:
        ...
        try:
            ...
            if "functions" in site_info:
                ...
                self.plugin_available = (
                    "local_moodleclaude_create_page_activity" in functions
                )
                if self.plugin_available:
                    logger.info(
                        "✅ MoodleClaude plugin detected - using enhanced functionality"
                    )
                else:
                    logger.warning(
                        "⚠️ MoodleClaude plugin not found - falling back to limited functionality"
                    )
                return self.plugin_available
    ...
    async def create_course(
        ...
        if not shortname:
            ...
            safe_name = "".join(
                c for c in name.lower()[:30] if c.isalnum() or c in (" ", "-", "_")
            ).replace(" ", "_")
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully created new course '{name}' with ID: {course_id}"
            )
    ...
    async def _enroll_user_in_course(self, course_id: int):
        ...
        try:
            ...
            logger.info(
                f"Enrolling user {username} (ID: {user_id}) in course {course_id}"
            )

            # Try self-enrollment
            enrol_methods = await self._call_api(
                "core_enrol_get_course_enrolment_methods", {"courseid": course_id}
            )

            for method in enrol_methods:
                if (
                    method.get("type") in ["self", "manual"]
                    and method.get("status") == 0
                ):
                    try:
                        await self._call_api(
                            "enrol_self_enrol_user",
                            {"courseid": course_id, "instanceid": method.get("id")},
                        )
                        logger.info(
                            f"Successfully enrolled user {username} in course {course_id}"
                        )
                        return
                    except Exception as e:
    ...
    async def create_section(
        self, course_id: int, name: str, description: str = "", position: int = 0
    ) -> int:
        """
        ...
        try:
            ...
            result = await self._call_api(
                MoodleWebServices.CREATE_SECTIONS,
                {"courseid": course_id, "position": position, "number": 1},
            )
            ...
            if result and len(result) > 0:
                ...
                if await self._check_plugin_availability():
                    await self.update_section_content(
                        course_id, section_number, name, description
                    )
    ...
    async def update_section_content(
        self, course_id: int, section_number: int, name: str, summary: str = ""
    ) -> bool:
        """
        ...
        if not await self._check_plugin_availability():
            logger.info(
                f"📝 Section {section_number} content (plugin required for storage):"
            )
    ...
    async def create_page_activity(
        ...
        """
        logger.info(
            f"📄 Creating page activity '{name}' for course {course_id}, section {section_id}"
        )
        ...
        try:
            ...
            if result.get("success"):
                ...
                return {
                    ...
                    "message": result.get(
                        "message", "Page activity created successfully"
                    ),
                }
            else:
                logger.warning(
                    f"❌ Page activity creation failed: {result.get('message')}"
                )
                return {
                    "success": False,
                    "method": "plugin_api",
    ...
    async def create_label_activity(
        ...
        """
        logger.info(
            f"🏷️ Creating label activity for course {course_id}, section {section_id}"
        )

        if not await self._check_plugin_availability():
            logger.warning(
                f"❌ Plugin not available - cannot create real label activity"
            )
            return {
                "success": False,
                "method": "plugin_required",
        ...
        try:
            result = await self._call_api(
                ...
                {"courseid": course_id, "section": section_id, "content": content},
            )
            ...
            if result.get("success"):
                ...
                return {
                    ...
                    "cm_id": result.get("cmid"),
                    "message": result.get(
                        "message", "Label activity created successfully"
                    ),
                }
            else:
                logger.warning(
                    f"❌ Label activity creation failed: {result.get('message')}"
                )
                return {
                    "success": False,
                    "method": "plugin_api",
    ...
    async def create_file_activity(
        ...
        """
        logger.info(
            f"📄 Creating file resource '{name}' ({filename}) for course {course_id}, section {section_id}"
        )
        ...
        try:
            ...
            if result.get("success"):
                ...
                logger.warning(
                    f"❌ File resource creation failed: {result.get('message')}"
                )
                return {
                    "success": False,
                    "method": "plugin_api",
    ...
    async def create_course_structure(
        self, course_id: int, sections_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"🏗️ Creating course structure for course {course_id} with {len(sections_data)} sections"
        )
        ...
        try:
            result = await self._call_api(
                "local_moodleclaude_create_course_structure",
                {"courseid": course_id, "sections": sections_data},
            )
            ...
            if result.get("success"):
                ...
                logger.warning(
                    f"❌ Course structure creation failed: {result.get('message')}"
                )
                return result

        except Exception as e:
            logger.error(f"❌ Error creating course structure: {e}")
            return {"success": False, "message": f"Error: {str(e)}", "sections": []}

    async def _create_structure_individually(
        self, course_id: int, sections_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
                        ...
                        try:
                            if activity_data["type"] == "page":
                                ...
                                result = await self.create_file_activity(
                                    ...
                                    activity_data["content"],
                                    activity_data.get(
                                        "filename", activity_data["name"] + ".txt"
                                    ),
                                )
                            else:
                                result = {
                                    "success": False,
                                    "message": f"Unknown activity type: {activity_data['type']}",
                                }
