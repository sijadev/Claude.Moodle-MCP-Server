...
from models import ChatContent, CourseStructure, ContentItem
        ...
        if self.config.moodle_url and self.config.moodle_token:
            try:
                self.moodle_client = MoodleClient(
                    ...
                    token=self.config.moodle_token
                )
...
"""
            ...
            return [types.TextContent(
                type="text",
                text=f"Failed to preview content: {str(e)}"
            )]
    
    async def _add_content_to_existing_course(self, arguments: Dict[str, Any]) -> List[types.TextContent]:
...
"""
            ...
            return [types.TextContent(
                type="text",
                text=f"Failed to add content to course: {str(e)}"
            )]
