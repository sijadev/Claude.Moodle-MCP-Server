    ...
    async def create_python_course_with_sections(self) -> dict:
        ...
        return {
            ...
            "summary": {
                ...
                "restricted_sections": len([s for s in sections if s['config'].availability_conditions])
            }
        }
...
# Das war's - Sie haben gerade programmiert! 🎉</code></pre>
    ...
    async def _create_page_resource(self, course_id: int, section_num: int, name: str, content: str):
        """Create a page resource with content"""
        
        try:
            # Get site info for context
            site_info = await self.api._make_request("core_webservice_get_site_info", {})
            ...
            return await self._create_label_resource(course_id, section_num, name, content)
    
    async def _create_label_resource(self, course_id: int, section_num: int, name: str, content: str):
    ...
    async def demonstrate_section_management(self, course_id: int, sections: list):
        ...
        try:
            ...
            await self.api.update_section(section_to_update['section_id'], {
    ...
    async def run_complete_demo(self):
        ...
        try:
            ...
            print(f"   🔒 Restricted sections: {result['summary']['restricted_sections']}")
