...
class ManualTestRunner:
    ...
    def run_test(self, test_file: str) -> Tuple[bool, str, float]:
        ...
        try:
            ...
            has_errors = any(indicator in result.stderr or indicator in result.stdout 
    ...
    def run_category(self, category: str) -> Dict[str, Tuple[bool, str, float]]:
        ...
        for test_file in self.test_categories[category]:
            ...
            if output:
                lines = output.split('\n')
                key_lines = [line for line in lines if any(indicator in line 
                           for indicator in ['✅', '❌', 'SUCCESS', 'FAILED', 'Created', 'ERROR'])]
    ...
    def print_summary(self, results: Dict[str, Dict[str, Tuple[bool, str, float]]]):
        ...
        for category, category_results in results.items():
            ...
            category_duration = sum(duration for _, _, duration in category_results.values())
        ...
        if total_passed == total_tests:
            ...
            print(f"   Common issues: Token problems, Docker not running, missing configuration")
    
    def print_detailed_output(self, results: Dict[str, Dict[str, Tuple[bool, str, float]]]):
...
def main():
    ...
    parser.add_argument('--category', choices=['core', 'advanced', 'validation', 'all'], 
    ...
    total_passed = sum(sum(1 for success, _, _ in cat_results.values() if success) 
