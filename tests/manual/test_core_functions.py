...
async def test_core_course_creation():
    ...
    try:
        async with MoodleClient(moodle_url, moodle_token) as client:
            ...
            try:
                result = await client._call_api("core_course_create_courses", course_data)
                ...
                print(f"   This confirms core_course_create_courses might be internal only")
            ...
            for course in courses[-3:]:  # Show last 3 courses
                print(f"   - ID {course.get('id', '?')}: {course.get('fullname', 'Unnamed')}")
