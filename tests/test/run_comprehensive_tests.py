...
class ComprehensiveTestSuite:
    ...
    def __init__(self):
        ...
        self.db_url = os.getenv('DATABASE_URL', 'postgresql://moodleuser:MoodleTestPass2025!@postgres_test:5432/moodletest')
    ...
    async def test_environment_validation(self) -> bool:
        ...
        for test_name, test_func in tests:
            try:
                ...
                self.log_test_result(f"env_{test_name}", result, 
                                   "Environment validation passed" if result else "Environment validation failed")
                if not result:
                    all_passed = False
    ...
    def _test_required_modules(self) -> bool:
        ...
        required_modules = [
            'requests', 'aiohttp', 'psycopg2', 'pytest', 
    ...
    def _test_environment_variables(self) -> bool:
        ...
        required_vars = [
            'MOODLE_URL', 'MOODLE_ADMIN_USER', 'MOODLE_ADMIN_PASSWORD',
    ...
    async def test_moodle_connectivity(self) -> bool:
        ...
        if not await self.wait_for_service(self.moodle_url):
            self.log_test_result("moodle_connectivity", False, "Moodle service not ready")
            return False
        ...
        try:
            ...
            if response.status_code != 200:
                self.log_test_result("moodle_basic_access", False, f"HTTP {response.status_code}")
                return False
            ...
            self.log_test_result("moodle_admin_login", login_success, 
                               "Admin login successful" if login_success else "Admin login failed")
            
            return login_success
            
        except Exception as e:
            self.log_test_result("moodle_connectivity", False, f"Connection error: {str(e)}")
            return False
    ...
    async def test_plugin_installation(self) -> bool:
        ...
        try:
            ...
            self.log_test_result("plugin_files_present", plugin_exists,
            ...
            self.log_test_result("plugin_structure", all_files_present,
                               "Plugin structure valid" if all_files_present else "Plugin structure invalid")
            
            return all_files_present
            
        except Exception as e:
            self.log_test_result("plugin_installation", False, f"Plugin test error: {str(e)}")
            return False
    ...
    async def test_webservice_setup(self) -> bool:
        ...
        try:
            ...
            self.log_test_result("webservice_accessible", webservice_accessible,
                               "Webservice endpoint accessible" if webservice_accessible else "Webservice endpoint not accessible")
            
            return webservice_accessible
            
        except Exception as e:
            self.log_test_result("webservice_setup", False, f"Webservice test error: {str(e)}")
            return False
    ...
    async def test_token_generation(self) -> bool:
        ...
        self.log_test_result("token_generation", True, "Token generation simulated", simulated_tokens)
        return True
        ...
        try:
            ...
            for test_name, module_name in tests:
                try:
                    ...
                    self.log_test_result(test_name, True, f"Successfully imported {module_name}")
                except ImportError as e:
                    self.log_test_result(test_name, False, f"Failed to import {module_name}: {str(e)}")
                    all_passed = False
            ...
            self.log_test_result("mcp_server_functionality", False, f"MCP test error: {str(e)}")
            return False
    ...
    async def _test_mcp_components(self) -> bool:
        ...
        try:
            ...
            processor.add_conversation_turn("test_session", "Create a test course", "success")
            ...
            self.log_test_result("context_processor_functionality", context_test_passed,
                               "Context processor working" if context_test_passed else "Context processor failed")
            
            # Test error handling
            from enhanced_error_handling import ErrorHandlerMixin, EnhancedError
            ...
            error = handler.create_error(
                category=handler.ErrorCategory.SYSTEM if hasattr(handler, 'ErrorCategory') else 'system',
                severity=handler.ErrorSeverity.LOW if hasattr(handler, 'ErrorSeverity') else 'low',
                title="Test Error",
                message="This is a test error"
            )
            
            error_test_passed = bool(error)
            self.log_test_result("error_handling_functionality", error_test_passed,
                               "Error handling working" if error_test_passed else "Error handling failed")
    ...
    async def test_performance_benchmarks(self) -> bool:
        ...
        try:
            ...
            performance_passed = processing_time < 1.0  # Should complete in under 1 second
            
            self.log_test_result("performance_benchmarks", performance_passed,
            ...
            self.log_test_result("performance_benchmarks", False, f"Performance test error: {str(e)}")
            return False
    ...
    async def test_integration_scenarios(self) -> bool:
        ...
        for test_name, test_func in integration_tests:
            try:
                ...
                self.log_test_result(f"integration_{test_name}", result,
                                   f"Integration test {test_name} passed" if result else f"Integration test {test_name} failed")
                if not result:
                    all_passed = False
            except Exception as e:
                self.log_test_result(f"integration_{test_name}", False, f"Integration test error: {str(e)}")
                all_passed = False
    ...
    def _test_content_parsing(self) -> bool:
        ...
        try:
            ...
            topics = [line.strip() for line in lines if line.strip().startswith(('1.', '2.', '3.', '4.'))]
    ...
    async def test_cleanup_validation(self) -> bool:
        ...
        try:
            ...
            for test_name, test_func in cleanup_tests:
                result = test_func()
                self.log_test_result(f"cleanup_{test_name}", result,
                                   f"Cleanup test {test_name} passed" if result else f"Cleanup test {test_name} failed")
                if not result:
                    all_passed = False
            ...
            self.log_test_result("cleanup_validation", False, f"Cleanup test error: {str(e)}")
            return False
    ...
    async def run_complete_test_suite(self) -> bool:
        ...
        for phase in self.test_phases:
            ...
            try:
                ...
                self.log_test_result(f"phase_{phase}", False, f"Phase crashed: {str(e)}")
                overall_success = False
