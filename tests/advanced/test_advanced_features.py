...
from src.core.adaptive_content_processor import (
    AdaptiveContentProcessor, ProcessingSession, SessionState, ProcessingStrategy
)
...
class TestAdaptiveContentProcessor(unittest.TestCase):
    ...
    def test_content_complexity_analysis(self):
        ...
        analysis = asyncio.run(self.processor.analyze_content_complexity(self.sample_content))
    ...
    def test_adaptive_limit_adjustment(self):
        ...
        self.assertGreaterEqual(
            self.processor.content_limits.max_char_length, 
            original_limit
        )
...
class TestIntelligentSessionManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_moodle_client.create_page_activity = AsyncMock(return_value={'id': 789})
        self.mock_moodle_client.get_course_contents = AsyncMock(return_value={
            'sections': [
                {'modules': [{'id': 1}, {'id': 2}]},
                {'modules': [{'id': 3}]}
            ]
    ...
    def test_session_continuation(self):
        ...
        self.assertTrue(continue_result.get('success', False) or 'error' in continue_result)
    ...
    def test_moodle_course_creation(self):
        ...
        result = asyncio.run(
            self.session_manager.create_intelligent_course_session(
                content=self.sample_content,
                course_name="Test Moodle Course"
            )
        )
...
class TestAdaptiveConfig(unittest.TestCase):
    ...
    def test_optimal_strategy_thresholds(self):
        ...
        self.assertLess(thresholds['intelligent_chunk'], thresholds['progressive_build'])
    ...
    def test_complete_workflow_simple_content(self):
        ...
        result = asyncio.run(
            session_manager.create_intelligent_course_session(
                ...
                course_name="Simple Python Course"
            )
        )
