...
class SimpleMoodleClaudeTestSuite:
    ...
    async def run_environment_check(self) -> bool:
        ...
        try:
            ...
            for dir_name in required_dirs:
                ...
                if dir_path.exists():
                    self.log_test_result(
                        f"directory_{dir_name}", True, f"Directory {dir_name} exists"
                    )
                else:
                    self.log_test_result(
            ...
            if requirements_file.exists():
                self.log_test_result(
                    "requirements_file", True, "requirements.txt found"
                )
            else:
                self.log_test_result(
                    "requirements_file", False, "requirements.txt missing"
                )
            ...
            self.log_test_result(
                "environment_check", False, f"Environment check failed: {str(e)}"
            )
            return False
    ...
    async def run_python_modules_test(self) -> bool:
        ...
        optional_modules = ["pydantic", "psycopg2", "requests", "pytest"]
    ...
    async def run_mcp_server_basic_test(self) -> bool:
        ...
        try:
            ...
            if not server_found:
                self.log_test_result(
                    "mcp_server_files", False, "No MCP server files found"
                )
                return False
    ...
    async def run_core_functionality_test(self) -> bool:
        ...
        try:
            ...
            self.log_test_result(
                "asyncio_basic", True, "Asyncio basic functionality working"
            )
            ...
            if parsed_data["test"] == "data":
                self.log_test_result(
                    "json_handling", True, "JSON serialization/deserialization working"
                )
            else:
                self.log_test_result(
                    "json_handling", False, "JSON handling failed validation"
                )
            ...
            if project_root.exists() and project_root.is_dir():
                self.log_test_result(
                    "path_handling", True, "Path handling working correctly"
                )
            else:
                self.log_test_result(
                    "path_handling", False, "Path handling issues detected"
                )
    ...
    async def run_performance_basic_test(self) -> bool:
        ...
        try:
            ...
            if memory_mb < 100:  # Less than 100MB
                self.log_test_result(
                    "memory_usage", True, f"Memory usage OK: {memory_mb:.1f}MB"
                )
            else:
                self.log_test_result(
                    "memory_usage", False, f"High memory usage: {memory_mb:.1f}MB"
                )
            ...
            self.log_test_result(
                "performance_basic_test", False, f"Performance test failed: {str(e)}"
            )
            return False
