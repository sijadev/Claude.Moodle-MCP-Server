...
class PerformanceMonitor:
    ...
    def analyze_configuration(self, config: Dict) -> Dict[str, Any]:
        ...
        for server_name, server_config in mcp_servers.items():
            ...
            if any(key.startswith(("CACHE_", "RATE_LIMIT_", "MAX_CONNECTIONS")) 
...
```
    ...
    async def run_benchmark(self, duration: int = 30) -> Dict[str, Any]:
        ...
        if response_times:
            benchmark_results["avg_response_time"] = sum(response_times) / len(response_times)
            benchmark_results["min_response_time"] = min(response_times)
            benchmark_results["max_response_time"] = max(response_times)
        
        actual_duration = time.time() - start_time
        benchmark_results["requests_per_second"] = benchmark_results["total_requests"] / actual_duration
...
async def main():
    ...
    if args.report:
        ...
        for check in health['checks']:
            status_emoji = {"healthy": "✅", "slow": "⚠️", "unhealthy": "❌"}.get(check['status'], "❓")
        ...
        print(f"• Active: {analysis['servers_configured'] - analysis['disabled_servers']}")
