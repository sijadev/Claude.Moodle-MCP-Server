...
class ComprehensiveTestRunner:
    ...
    def run_command(self, command: list, description: str, timeout: int = 300) -> tuple[bool, str]:
    ...
    def check_environment(self) -> bool:
        ...
        try:
            result = subprocess.run(['docker', 'ps', '--filter', 'name=moodleclaude'], 
        ...
        try:
            result = subprocess.run(['uv', 'run', 'python', '-c', 'import mcp_server'], 
                                  capture_output=True, text=True)
...
def main():
    ...
    parser.add_argument('--suite', choices=['unit', 'integration', 'manual', 'e2e', 'all'], 
                       default='all', help='Test suite to run')
    parser.add_argument('--manual-category', choices=['core', 'advanced', 'validation', 'all'],
                       default='all', help='Manual test category')
