...
async def verify_tokens():
    ...
    try:
        async with MoodleClient(config.moodle_url, config.get_basic_token()) as basic_client:
    ...
    try:
        async with EnhancedMoodleClient(config.moodle_url, plugin_token) as plugin_client:
            ...
            if plugin_available:
                ...
                if page_result['success']:
                    print(f"   ✅ Page activity creation works! (ID: {page_result['activity_id']})")
                    print(f"   🔗 Visit: {config.moodle_url}/course/view.php?id={test_course_id}")
                else:
                    print(f"   ❌ Page activity creation failed: {page_result['message']}")
                    
            else:
                print("   ❌ Plugin functions not available")
                print("   💡 Check if MoodleClaude service is enabled and token is correct")
