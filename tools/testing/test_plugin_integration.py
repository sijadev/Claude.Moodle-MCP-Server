    ...
    async with EnhancedMoodleClient(config.moodle_url, config.moodle_token) as client:
        ...
        try:
            ...
            if label_result["success"]:
                print(
                    f"✅ Label activity created successfully (ID: {label_result['activity_id']})"
                )
            else:
...
"""
            ...
            if file_result["success"]:
                print(
                    f"✅ File resource created successfully (ID: {file_result['activity_id']})"
                )
            else:
        ...
        try:
            ...
            structure_result = await client.create_course_structure(
                course_id, sections_data
            )
            if structure_result["success"]:
                ...
                total_activities = sum(
                    ...
                    for s in structure_result.get("sections", [])
                )
                print(f"   📚 Sections created: {sections_created}")
                print(f"   📄 Activities created: {total_activities}")
            else:
                print(
                    f"❌ Bulk structure creation failed: {structure_result['message']}"
                )
        except Exception as e:
