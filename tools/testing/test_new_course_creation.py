...
async def test_new_course_creation():
    ...
    async with MoodleClient(config.moodle_url, config.moodle_token) as client:
        ...
        test_description = "This is a test course to verify new course creation works properly."
        ...
        try:
            ...
            # Find the new course
            new_course = next((c for c in courses_after if c.get('id') == new_course_id), None)
            ...
            if new_course:
                ...
                if courses_before and new_course_id > highest_id_before:
                    print(f"✅ New course has proper sequential ID ({new_course_id} > {highest_id_before})")
                else:
...
async def main():
    ...
    if new_course_id:
        ...
        print(f"docker exec moodleclaude_db mariadb -u root bitnami_moodle -e \"SELECT id, fullname, shortname, summary FROM mdl_course WHERE id = {new_course_id};\"")
