...
def cleanup_docker_environment():
    ...
    for container in containers:
        run_command(
            ...
            f"Removing container {container}",
        )
    ...
    for volume in volumes:
        run_command(
            f"docker volume rm {volume} 2>/dev/null || true",
...
async def setup_fresh_moodle():
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(moodle_url, timeout=10) as response:
                    if response.status == 200:
                        ...
                        print(
                            f"⚠️ Moodle HTTP {response.status} (attempt {attempt + 1}/{max_retries})"
                        )
        except Exception as e:
    ...
    print(
        "   7. The service should be enabled with 9 functions (5 MoodleClaude + 4 core)"
    )
    ...
    print("\n💡 **Troubleshooting:**")
    print(
        "   • If plugin not detected: docker exec moodleclaude_app php /bitnami/moodle/admin/cli/upgrade.php"
    )
