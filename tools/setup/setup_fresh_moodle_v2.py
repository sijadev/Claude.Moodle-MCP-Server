...
?>"""
    ...
    commands = [
        (
            f"docker cp {script_path} moodleclaude_app_fresh:/tmp/create_wsuser_setup.php",
            "Copy user creation script",
        ),
        (
            "docker exec moodleclaude_app_fresh php /tmp/create_wsuser_setup.php",
            f"Create webservice user: {ws_user}",
        ),
        (
            "docker exec moodleclaude_app_fresh rm /tmp/create_wsuser_setup.php",
            "Cleanup script",
        ),
    ]
...
def install_plugin():
    ...
    plugin_commands = [
        (
            "docker cp moodle_plugin/local_moodleclaude/. moodleclaude_app_fresh:/opt/bitnami/moodle/local/moodleclaude/",
...
def generate_api_tokens():
    ...
    try:
        ...
        result = subprocess.run(
            admin_token_cmd, shell=True, capture_output=True, text=True
        )
...
?>"""
                ...
                token_commands = [
                    (
                        f"docker cp {script_path} moodleclaude_app_fresh:/tmp/generate_wsuser_token.php",
                        "Copy token script",
                    ),
                    (
                        "docker exec moodleclaude_app_fresh php /tmp/generate_wsuser_token.php",
                        "Generate wsuser token",
                    ),
                ]
                ...
                try:
                    os.unlink(script_path)
                    run_command(
                        "docker exec moodleclaude_app_fresh rm /tmp/generate_wsuser_token.php",
                        "Cleanup token script",
                    )
                except:
                    pass
...
"'''
    ...
    try:
        config_validation = subprocess.run(
...
def create_default_backup():
    ...
    try:
        ...
        db_backup_cmd = (
            "docker exec moodleclaude_postgres_fresh pg_dump -U moodle -d moodle_fresh"
        )
...
def main():
    ...
    parser.add_argument(
        "--quick-setup", action="store_true", help="Skip confirmation prompts"
    )
    parser.add_argument(
        "--force-rebuild", action="store_true", help="Force complete rebuild"
    )
    parser.add_argument(
        "--skip-cleanup", action="store_true", help="Skip container cleanup"
    )
    parser.add_argument(
        "--config-only", action="store_true", help="Only generate configuration"
    )
    ...
    if not args.quick_setup:
        confirm = input(
            "This will delete existing MoodleClaude data. Continue? (y/N): "
        )
        if confirm.lower() != "y":
