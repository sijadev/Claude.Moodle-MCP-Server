...
class OptimizedMoodleSetup:
    ...
    def run_command(self, cmd: str, description: str = "", capture_output: bool = True) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_step(f"Failed: {description} - {result.stderr.strip()[:200] if result.stderr else 'Unknown error'}", False)
                return False
    ...
    def setup_docker_environment(self, force_rebuild: bool = False) -> bool:
        ...
        if force_rebuild:
            self.run_command("docker-compose -f operations/docker/docker-compose.fresh.yml down -v", 
                           "Removing existing containers")
        
        # Start fresh containers
        compose_file = "operations/docker/docker-compose.fresh.yml"
        if not self.run_command(f"docker-compose -f {compose_file} up -d", 
        ...
        for attempt in range(max_attempts):
            if self.run_command("curl -s http://localhost:8080 > /dev/null", 
    ...
    def setup_moodle_configuration(self) -> bool:
        ...
        if not self.run_command(f'docker exec moodle_app bash -c "{admin_setup_script}"',
        ...
        if not self.run_command(f'docker exec moodle_app bash -c "{webservice_script}"',
    ...
    def install_moodleclaude_plugin(self) -> bool:
        ...
        copy_cmd = f"docker cp {plugin_source} moodle_app:/var/www/html/local/moodleclaude"
    ...
    def create_webservice_users_and_tokens(self) -> Dict[str, str]:
        ...
        self.run_command(f'docker exec moodle_app bash -c "{create_user_script}"',
        ...
        result = subprocess.run(f'docker exec moodle_app bash -c "{token_script}"',
        ...
        self.log_step(f"Generated tokens: Admin={bool(tokens.get('admin'))}, WSUser={bool(tokens.get('wsuser'))}")
        return tokens
    ...
    def update_environment_with_tokens(self, tokens: Dict[str, str]) -> bool:
        ...
        try:
            ...
            # Update tokens
            if tokens.get('admin'):
                env_content = self._update_env_var(env_content, 'MOODLE_BASIC_TOKEN', tokens['admin'])
                env_content = self._update_env_var(env_content, 'MOODLE_ADMIN_TOKEN', tokens['admin'])
            
            if tokens.get('wsuser'):
                env_content = self._update_env_var(env_content, 'MOODLE_PLUGIN_TOKEN', tokens['wsuser'])
                env_content = self._update_env_var(env_content, 'MOODLE_WSUSER_TOKEN', tokens['wsuser'])
    ...
    def setup_optimized_claude_desktop_config(self) -> bool:
        ...
        try:
            ...
            template_path = self.project_root / "config" / "claude_desktop_optimized.json"
            ...
            if not claude_config_dir:
                self.log_step("Could not determine Claude Desktop config directory", False)
                return False
            ...
            if claude_config_file.exists():
                backup_path = claude_config_dir / f"claude_desktop_config.json.backup.{int(time.time())}"
    ...
    def run_full_setup(self, skip_docker: bool = False, force_rebuild: bool = False) -> bool:
...
def main():
    ...
    parser.add_argument("--setup-only-claude-config", action="store_true",
