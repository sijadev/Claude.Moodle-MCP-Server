...
class TokenPermissionsFixer:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "exception" not in data:
                    ...
                    logger.warning(
                        f"  ❌ Site info failed: {data.get('message', 'Unknown error')}"
                    )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, list) or (
                    ...
                    logger.warning(
                        f"  ❌ Course listing failed: {data.get('message', 'Unknown error')}"
                    )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, list) and len(data) > 0 and "id" in data[0]:
                    ...
                    logger.warning(
                        f"  ❌ Course creation failed: {data.get('message', 'Unknown error')}"
                    )
                else:
    ...
    def create_admin_token(self):
        ...
        try:
            ...
            logintoken_match = re.search(
                r'name="logintoken" value="([^"]*)"', login_page.text
            )
            ...
            login_response = session.post(
                f"{moodle_url}/login/index.php", data=login_data
            )

            if (
                "Dashboard" in login_response.text
                or "invalid login" not in login_response.text.lower()
            ):
    ...
    def fix_permissions(self):
        ...
        for token_name, token_value in tokens_to_test.items():
            if token_value:
                ...
                if user_info:
                    logger.info(
                        f"  👤 User: {user_info.get('fullname', 'Unknown')} ({user_info.get('username', 'Unknown')})"
                    )
        ...
        for token_name, result in results.items():
            ...
            if result["user_info"]:
                logger.info(
                    f"   👤 User: {result['user_info']['fullname']} ({result['user_info']['username']})"
                )
        ...
        if best_capabilities.get("create_courses", False):
            ...
            if claude_config_path.exists():
                try:
                    ...
                    if (
                        ...
                        and "moodleclaude-stable" in claude_config["mcpServers"]
                    ):
                        server_config = claude_config["mcpServers"][
                            "moodleclaude-stable"
                        ]
                        server_config["env"]["MOODLE_TOKEN_ENHANCED"] = results[
                            best_token
                        ]["token"]
                        ...
                        logger.info(
                            f"✅ Updated Claude Desktop config with {best_token}"
                        )
            ...
            logger.info(
                "   3. Ensure course creation is enabled for the web service user"
            )
