...
def test_workflow_components():
    ...
    try:
        ...
        assert config.credentials.admin_password == "MoodleClaude2025!", "Wrong admin password"
    ...
    try:
        ...
        with open(env_file) as f:
            env_content = f.read()
            assert "MOODLE_ADMIN_PASSWORD=MoodleClaude2025!" in env_content, "Wrong password in .env"
    ...
    try:
        ...
        with open(mcp_launcher) as f:
            ...
            assert "server_dir = os.path.dirname(os.path.abspath(__file__))" in launcher_content, "Path fix not applied"
            assert "project_root = os.path.dirname(server_dir)" in launcher_content, "Project root fix not applied"
    ...
    total_tests += 1
    try:
        docker_compose = PROJECT_ROOT / "operations" / "docker" / "docker-compose.fresh.yml"
        assert docker_compose.exists(), "Fresh docker-compose not found"
        
        with open(docker_compose) as f:
            compose_content = f.read()
            assert "MOODLE_PASSWORD: MoodleClaude2025!" in compose_content, "Wrong password in docker-compose"
            assert "PGADMIN_DEFAULT_PASSWORD: MoodleClaude2025!" in compose_content, "Wrong pgadmin password"
    ...
    try:
        ...
        with open(setup_guide) as f:
            ...
            assert "python tools/setup/setup_fresh_moodle_v2.py --quick-setup" in guide_content, "Quick setup command missing"
