...
class SetupUpdater:
    ...
    def update_claude_desktop_config(self) -> bool:
        ...
        if CLAUDE_CONFIG_PATH.exists():
            backup_path = CLAUDE_CONFIG_PATH.with_suffix(f".backup.{int(datetime.now().timestamp())}")
    ...
    def validate_fixes(self) -> Dict[str, bool]:
        ...
        if CLAUDE_CONFIG_PATH.exists():
            try:
                ...
                command = config.get('mcpServers', {}).get('moodleclaude-stable', {}).get('command', '')
                validation_results['python_path_absolute'] = os.path.isabs(command)
                
                # Check if Python executable exists
                validation_results['python_executable_exists'] = Path(command).exists() if command else False
        ...
        try:
            import requests
            response = requests.get(self.config.get('MOODLE_URL', 'http://localhost:8080'), timeout=5)
    ...
    def run_update(self, claude_config_only: bool = False, moodle_permissions_only: bool = False) -> bool:
        ...
        if success:
            ...
            if 'moodle_permissions' in applied_fixes and applied_fixes['moodle_permissions']:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update MoodleClaude Setup with Bug Fixes")
    parser.add_argument("--claude-config-only", action="store_true",
                       help="Update only the Claude Desktop configuration")
    parser.add_argument("--moodle-permissions-only", action="store_true",
