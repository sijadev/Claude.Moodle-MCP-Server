                    ...
                    for line in stdout.split('\n')[:5]:  # First 5 lines
                        if line.strip():
                            try:
                                ...
                                print(f"   Response ID {response.get('id', 'N/A')}: {response.get('result', response.get('error', 'Unknown'))}")
                            except:
...
def check_claude_config():
    ...
    try:
        ...
        enabled_servers = [name for name, cfg in servers.items() if not cfg.get('disabled', False)]
