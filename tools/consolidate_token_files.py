...
class TokenFileConsolidator:
    ...
    def analyze_token_files(self):
        ...
        token_files = ["moodle_tokens.env", "moodle_tokens.env"]
        ...
        for token_file in token_files:
            ...
            if file_path.exists():
                ...
                logger.info(
                    f"   Modified: {datetime.fromtimestamp(file_info[token_file]['mtime']).strftime('%Y-%m-%d %H:%M:%S')}"
                )
    ...
    def consolidate_identical_files(self, file_info: dict):
        ...
        for file_hash, files in hash_groups.items():
            if len(files) > 1:
                logger.info(
                    f"🔍 Found {len(files)} identical files (hash: {file_hash[:16]}...)"
                )
            ...
            if current_file != "moodle_tokens.env":
                ...
                if canonical_path.exists():
                    ...
                    backup_path = canonical_path.with_suffix(
                        f".env.backup.{int(datetime.now().timestamp())}"
                    )
                    shutil.move(canonical_path, backup_path)
                    logger.info(
                        f"💾 Backed up existing canonical file to: {backup_path.name}"
                    )
    ...
    def update_references(self):
        ...
        search_patterns = ["moodle_tokens.env", "moodle_tokens.env"]
        ...
        for py_file in python_files:
            try:
                ...
                if content != original_content:
                    ...
                    logger.info(
                        f"🔧 Updated references in: {py_file.relative_to(self.project_root)}"
                    )
        ...
        if updated_files:
            self.actions_taken.append(
                {"action": "updated_references", "files": updated_files}
            )
        else:
        ...
        if report["summary"]["total_actions"] > 0:
            ...
            for action in self.actions_taken:
                if action["action"] == "removed_duplicate":
                    ...
                    logger.info(
                        f"  📝 Renamed: {action['old_name']} → {action['new_name']}"
                    )
