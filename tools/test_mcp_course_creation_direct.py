        ...
        try:
            async with session.post(url, data=data) as response:
                ...
                if isinstance(result, dict) and 'exception' in result:
                    ...
                    async with session.post(url, data=contents_data) as contents_response:
                        contents_result = await contents_response.json()
                        
                        if isinstance(contents_result, list):
                            print(f"   ✅ Kurs-Inhalte abgerufen: {len(contents_result)} Abschnitte")
                            for i, section in enumerate(contents_result[:3]):  # Zeige erste 3
                                print(f"      - Abschnitt {i}: {section.get('name', 'Unnamed')}")
                        else:
                            print(f"   ⚠️ Unerwartetes Inhalts-Format: {contents_result}")
