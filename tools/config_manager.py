...
def generate_env_files():
    ...
    files_to_generate = [
        PROJECT_ROOT / ".env",
        PROJECT_ROOT / "config" / "moodle_tokens.env",
        PROJECT_ROOT / "config" / "moodle_tokens.env",
    ]
...
def update_claude_desktop_config(use_optimized: bool = True):
    ...
    if use_optimized:
        ...
        optimized_server_path = (
            project_root / "src" / "core" / "optimized_mcp_server.py"
        )
...
def main():
    ...
    subparsers.add_parser(
        "update-claude-desktop", help="Update Claude Desktop configuration"
    )
