...
async def test_token(token_name: str, token_value: str):
    ...
    async with MoodleAPIClient(MOODLE_URL, token_value) as client:
        try:
            ...
            print(
                f"   ✅ Site Info: {site_info.get('fullname')} ({site_info.get('username')})"
            )
            ...
            try:
                ...
                print(
                    f"   ✅ Kategorien abrufen: {len(categories)} Kategorien gefunden"
                )
            except Exception as e:
            ...
            try:
                ...
                result = await client.call_webservice(
                    "core_course_create_courses", courses=test_course_data
                )
                ...
                if result and len(result) > 0:
                    ...
                    try:
                        await client.call_webservice(
                            "core_course_delete_courses", courseids=[course_id]
                        )
