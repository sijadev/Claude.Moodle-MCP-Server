...
MOODLE_TOKEN = config.get('MOODLE_PLUGIN_TOKEN', config.get('MOODLE_BASIC_TOKEN', config.get('MOODLE_ADMIN_TOKEN', '')))
...
async def check_webservice_capabilities():
    ...
    async with MoodleAPIClient(MOODLE_URL, MOODLE_TOKEN) as client:
        try:
            ...
            course_functions = [f for f in functions if 'course' in f.get('name', '').lower()]
