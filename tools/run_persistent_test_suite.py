...
from core.test_data_storage import TestDataStorage, TestResult, BenchmarkData, create_test_data_storage
...
class PersistentMoodleClaudeTestSuite:
    ...
    def log_and_store_test_result(self, test_name: str, success: bool, message: str, 
    ...
    def store_benchmark(self, test_name: str, metric_name: str, value: float, 
                       unit: str = "seconds", metadata: Optional[Dict] = None):
    ...
    async def run_environment_check_with_persistence(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                test_data = {'test': 'storage_check', 'timestamp': datetime.now().isoformat()}
                if self.storage.store_json_data('storage_test', test_data):
                    ...
                    self.log_and_store_test_result(
                        'storage_system_check',
                        False,
                        "Storage system write failed"
                    )
            except Exception as e:
                self.log_and_store_test_result(
                    'storage_system_check',
                    False,
                    f"Storage system error: {str(e)}"
                )
            ...
            # Store benchmark
            self.store_benchmark('environment_check', 'check_duration', duration, 'seconds')
    ...
    async def run_module_import_tests_with_benchmarks(self) -> bool:
        ...
        core_modules = [
            'asyncio', 'aiohttp', 'json', 'logging', 'pathlib', 'datetime'
        ]
        
        optional_modules = [
            'pydantic', 'requests', 'pytest', 'sqlite3'
        ]
    ...
    async def run_performance_benchmarks(self) -> bool:
        ...
        try:
            ...
            self.store_benchmark('json_processing', 'serialize_time', serialize_duration, 'seconds')
            self.store_benchmark('json_processing', 'deserialize_time', deserialize_duration, 'seconds')
        ...
        try:
            ...
            for key, benchmarks in benchmark_groups.items():
                if len(benchmarks) >= 2:
                    ...
                    change_percent = ((latest.value - previous.value) / previous.value * 100) if previous.value > 0 else 0
            ...
            self.storage.store_json_data(f'trend_analysis_{self.session_id}', trend_analysis)
    ...
    async def run_all_tests(self) -> bool:
        ...
        if trend_analysis.get('performance_regression'):
            ...
            for regression in trend_analysis['performance_regression']:
                self.logger.warning(f"  - {regression['test']}: {regression['change_percent']:+.1f}%")
...
async def main():
    ...
    parser.add_argument('--cleanup', action='store_true', help='Cleanup old test data (30+ days)')
    parser.add_argument('--snapshot', type=str, help='Create snapshot of test data')
    parser.add_argument('--stats', action='store_true', help='Show test statistics only')
