...
from core.working_mcp_server import handle_test_connection, handle_get_courses, handle_create_course, handle_get_course_contents
        ...
        if result and len(result) > 0:
            ...
            if "Course created successfully" in response:
                ...
                if id_match:
                    ...
                    print("\n📖 Test 4: Get Course Contents")
                    try:
                        contents_result = await handle_get_course_contents({"course_id": course_id})
                        if contents_result and len(contents_result) > 0:
                            contents_response = contents_result[0].text
                            if "Course contents" in contents_response or "sections" in contents_response:
                                print("✅ Get course contents test passed")
                                test_results.append(("Get Course Contents", "PASS"))
                            else:
                                print(f"❌ Unexpected contents response: {contents_response}")
