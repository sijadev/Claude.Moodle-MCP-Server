...
class ConfigLogCleaner:
    ...
    def remove_file_or_dir(self, path: Path, reason: str = ""):
        ...
        try:
            if path.is_file():
                ...
                logger.info(
                    f"🗑️  Removed directory: {path.name} ({size:,} bytes) - {reason}"
                )
            ...
            for config_file in config_dir.iterdir():
                if config_file.is_file():
                    if config_file.name in essential_configs:
                        ...
                        if (
                            "advanced" in config_file.name
                            or "basic" in config_file.name
                            or "simple_test" in config_file.name
                        ):
                            self.remove_file_or_dir(
                                config_file, "Duplicate/obsolete config"
                            )
                        else:
                        ...
                        if any(
                            keyword in config_file.name.lower()
                            for keyword in ["token", "dual", "adaptive"]
                        ):
    ...
    def cleanup_old_logs(self, keep_days: int = 7):
        ...
        log_dirs = [self.project_root / "logs", self.project_root]
            ...
            for log_file in log_dir.glob("*.log"):
                if log_file.is_file():
                    ...
                    if file_time < cutoff_date:
                        self.remove_file_or_dir(
                            log_file, f"Log older than {keep_days} days"
                        )
                    else:
                        self.keep_file(
                            log_file, f"Recent log ({file_time.strftime('%Y-%m-%d')})"
                        )
    ...
    def cleanup_test_artifacts(self):
        ...
        test_files = list(self.project_root.glob("*test*.json")) + list(
            self.project_root.glob("*test*.log")
        )
    ...
    def cleanup_backup_duplicates(self):
        ...
        backup_dirs = [
            d for d in backups_dir.iterdir() if d.is_dir() and "moodleclaude_" in d.name
        ]
    ...
    def cleanup_obsolete_servers(self):
        ...
        essential_servers = ["working_mcp_server.py", "test_data_storage.py"]
        ...
        report = {
            ...
            "summary": {
                ...
                "total_size_removed_mb": round(
                    self.total_size_removed / (1024 * 1024), 2
                ),
            },
    ...
    def run_cleanup(self):
        ...
        if (
            report["summary"]["files_removed"] > 0
            or report["summary"]["directories_removed"] > 0
        ):
