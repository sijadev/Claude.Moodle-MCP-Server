...
class DockerTestSuiteRunner:
    ...
    def check_docker_availability(self) -> bool:
        ...
        try:
            ...
            result = self.run_command(
                "docker-compose --version", "Checking Docker Compose"
            )
            if result.returncode != 0:
                self.log_phase(
                    "docker_compose_check", False, "Docker Compose not available"
                )
                return False
            ...
            if result.returncode != 0:
                self.log_phase(
                    "docker_daemon_check", False, "Docker daemon not running"
                )
                return False
            ...
            self.log_phase(
                "docker_availability", False, f"Docker check failed: {str(e)}"
            )
            return False
    ...
    def cleanup_existing_environment(self) -> bool:
        ...
        try:
            ...
            result = self.run_command(
                cleanup_cmd, "Stopping existing containers", timeout=120
            )
            ...
            for container in self.test_containers:
                self.run_command(
                    ...
                    f"Force removing {container}",
                    capture_output=True,
                )

            # Remove test network if it exists
            self.run_command(
                f"docker network rm {self.test_network}",
    ...
    def create_test_environment(self) -> bool:
        ...
        try:
            ...
            result = self.run_command(
                build_cmd, "Building test runner image", timeout=600
            )

            if result.returncode != 0:
                self.log_phase(
                    "build_test_image", False, "Failed to build test runner image"
                )
                return False

            # Start database and Moodle services
            services_cmd = (
                f"docker-compose -f {self.compose_file} up -d postgres_test moodle_test"
            )
            result = self.run_command(
                services_cmd, "Starting database and Moodle services", timeout=300
            )
            ...
            self.log_phase(
                "create_environment", True, "Test environment created successfully"
            )
            return True

        except Exception as e:
            self.log_phase(
                "create_environment", False, f"Environment creation failed: {str(e)}"
            )
            return False
    ...
    def _wait_for_services(self, timeout: int = 300) -> bool:
        ...
        while time.time() - start_time < timeout:
            ...
            for service in services:
                ...
                result = self.run_command(
                    health_cmd, f"Checking {service} health", capture_output=True
                )
    ...
    def run_test_suite(self) -> bool:
        ...
        try:
            ...
            result = self.run_command(
                test_cmd, "Running test suite", capture_output=False, timeout=1800
            )  # 30 minutes
            ...
            if success:
                self.log_phase(
                    "test_execution", True, "Test suite completed successfully"
                )
            else:
    ...
    def _copy_test_results(self):
        ...
        try:
            ...
            self.run_command(
                create_cmd, "Creating temporary container for results copy"
            )
            ...
            self.run_command(
                f"docker rm {temp_container}", "Removing temporary container"
            )
    ...
    def cleanup_test_environment(self) -> bool:
        ...
        try:
            ...
            result = self.run_command(
                cleanup_cmd, "Cleaning up test environment", timeout=120
            )

            if result.returncode == 0:
                self.log_phase(
                    "cleanup", True, "Test environment cleaned up successfully"
                )
                return True
            else:
    ...
    def generate_final_report(self, analysis: Dict[str, Any]):
        ...
        runner_report_path = (
            self.project_root / "test-results" / "docker_test_runner_report.json"
        )
    ...
    def run_complete_test_cycle(self) -> bool:
        ...
        try:
            ...
            overall_success = (
                test_success
                and analysis.get("success_rate", 0) >= 80
                and len(analysis.get("critical_failures", [])) == 0
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MoodleClaude Docker Test Suite Runner"
    )
    parser.add_argument(
        "--keep-environment",
        action="store_true",
