...
class MCPServerDiagnostics:
    ...
    def check_claude_desktop_config(self):
        ...
        if not self.claude_config_path.exists():
            logger.error(f"❌ Claude Desktop config not found: {self.claude_config_path}")
            return False
    ...
    def test_python_import(self):
        ...
        try:
            ...
            result = subprocess.run([
                'python', '-c', f'import sys; sys.path.insert(0, "{self.mcp_server_path.parent}"); exec(open("{self.mcp_server_path}").read())'
                ...
                with open(token_file, 'r') as f:
                    for line in f:
                        if '=' in line and not line.strip().startswith('#'):
                            ...
                            if 'TOKEN' in key:
                                logger.info(f"   {key}: {'✅ Loaded' if value.strip() else '❌ Empty'}")
            except Exception as e:
        ...
        for log_file in log_files:
            try:
                ...
                for line in recent_lines:
                    if any(keyword in line.lower() for keyword in ['mcp', 'server', 'disconnect', 'error', 'fail']):
    ...
    def create_diagnostic_report(self):
        ...
        if not report['mcp_server_file']:
            report['recommendations'].append("Check MCP Server file permissions and location")
        ...
        if not report['log_files_found']:
            report['recommendations'].append("Enable Claude Desktop logging to diagnose connection issues")
        ...
        if passed_checks == total_checks:
            ...
            logger.info("❌ Some diagnostics failed - this explains the 'Server disconnected' error")
