...
def backup_config(config_path: Path):
    ...
    if config_path.exists():
        backup_path = config_path.with_suffix(
            f".json.backup.{int(os.path.getmtime(config_path))}"
        )
    ...
    config = {
        "mcpServers": {
            "moodleclaude-stable": {
                ...
                "timeout": 30,
                "autoApprove": ["test_connection", "get_courses"],
                "disabled": False,
...
def fix_claude_desktop_config():
    ...
    try:
        ...
        print(
            f"✅ Configuration validated - {len(loaded_config.get('mcpServers', {}))} servers configured"
        )
    ...
    try:
        ...
        result = subprocess.run(
            [
                "python",
                "-c",
                f'import sys; sys.path.append("{project_root / "src"}"); exec(open("{working_server}").read())',
            ],
            capture_output=True,
            text=True,
            timeout=5,
    ...
    if success:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    "⚠️ Moodle may not be running - start with: python tools/setup/start_mcp_server.py"
                )
        except:
            print(
                "⚠️ Moodle doesn't appear to be running - start with Docker or local setup"
            )
