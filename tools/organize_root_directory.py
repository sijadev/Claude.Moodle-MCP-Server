...
class RootDirectoryOrganizer:
    ...
    def move_file(self, source: Path, destination: Path, reason: str = ""):
        ...
        try:
            ...
            logger.info(
                f"📦 Moved: {source.name} → {destination.parent.name}/{destination.name} - {reason}"
            )
            return True
        ...
        for script_name in shell_scripts:
            ...
            if source.exists():
                if "backup" in script_name.lower() or "restore" in script_name.lower():
                    destination = (
                        self.project_root / "operations" / "backup" / script_name
                    )
                else:
    ...
    def organize_log_and_result_files(self):
        ...
        log_files = ["persistent_test_suite.log", "simple_test_suite.log"]
        ...
        result_files = ["simple_test_results.json", "cleanup_report.json"]
    ...
    def organize_executable_files(self):
        ...
        executable_files = ["start_server.py"]
        ...
        for root, dirs, files in os.walk(self.project_root, topdown=False):
            for dir_name in dirs:
                ...
                try:
                    ...
                    contents = [
                        f for f in dir_path.iterdir() if not f.name.startswith(".")
                    ]
                    if not contents:
                        dir_path.rmdir()
                        logger.info(
