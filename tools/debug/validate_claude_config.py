            ...
            if 'moodle-course-creator' in servers:
                ...
                for field in required_fields:
                    if field in moodle_config:
                        print(f"   ✅ {field}: {moodle_config[field] if field != 'env' else '[configured]'}")
                    else:
                ...
                if 'env' in moodle_config:
                    ...
                    required_env = ['MOODLE_URL', 'MOODLE_BASIC_TOKEN', 'MOODLE_PLUGIN_TOKEN', 'MOODLE_USERNAME']
                    ...
                    for var in required_env:
                        if var in env:
                            value = env[var]
                            if 'TOKEN' in var:
                                print(f"      ✅ {var}: ...{value[-8:] if len(value) > 8 else 'set'}")
                            else:
                                print(f"      ✅ {var}: {value}")
                        else:
                            print(f"      ❌ Missing: {var}")
