...
async def check_database_after_creation(course_id):
    ...
    print(
        f'   docker exec moodleclaude_db mariadb -u root bitnami_moodle -e "SELECT id, fullname, shortname, summary FROM mdl_course WHERE id = {course_id};"'
    )
    print(
        f'   docker exec moodleclaude_db mariadb -u root bitnami_moodle -e "SELECT course, section, name, summary FROM mdl_course_sections WHERE course = {course_id} ORDER BY section;"'
    )
