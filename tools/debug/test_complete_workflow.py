...
async def test_complete_workflow():
    ...
    print(
        f"Configuration: {server.config.get_config_summary() if server.config else 'No config'}"
    )
    ...
    try:
        ...
        if result and len(result) > 0:
            ...
            if (
                ...
                or "Enhanced Course Created Successfully" in response_text
            ):
