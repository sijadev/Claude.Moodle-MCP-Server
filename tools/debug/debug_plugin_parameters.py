...
async def debug_plugin_parameters():
    ...
    async with plugin_client as client:
        ...
        test_params = {
            'courseid': course_id,
            'sections': test_sections_data
        }
        
        print("Parameter types:")
        print(f"  courseid: {type(test_params['courseid'])} = {test_params['courseid']}")
        print(f"  sections: {type(test_params['sections'])} with {len(test_params['sections'])} items")
        ...
        for i, section in enumerate(test_params['sections']):
            ...
            print(f"      activities: {type(section['activities'])} with {len(section['activities'])} items")
            
            for j, activity in enumerate(section['activities']):
                print(f"        Activity {j}:")
                print(f"          type: {type(activity['type'])} = '{activity['type']}'")
                print(f"          name: {type(activity['name'])} = '{activity['name']}'")
                print(f"          content: {type(activity['content'])} ({len(activity['content'])} chars)")
                print(f"          filename: {type(activity['filename'])} = '{activity['filename']}'")
