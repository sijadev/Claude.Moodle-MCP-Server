...
async def debug_get_sections():
    ...
    async with MoodleClient(config.moodle_url, config.moodle_token) as client:
        ...
        try:
            contents_result = await client._call_api(
        ...
        try:
            sections_result = await client._call_api(
                MoodleWebServices.GET_SECTIONS, {"courseid": course_id}
            )
        ...
        try:
            course_sections_result = await client._call_api(
                "core_course_get_course_sections", {"courseid": course_id}
            )
