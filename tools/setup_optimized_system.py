...
class MinimalConfigManager:
    def validate_all(self):
        return {
            ...
            "message": "Full config manager not available",
        }
...
class OptimizedSystemSetup:
    ...
    def check_prerequisites(self) -> Dict[str, bool]:
        ...
        checks = {
            ...
            "config_files": (
                self.project_root / "config" / "master_config.py"
            ).exists(),
            "environment_file": (self.project_root / ".env").exists(),
            "optimized_server": (
                self.project_root / "src" / "core" / "optimized_mcp_server.py"
            ).exists(),
            "error_handling": (
                self.project_root / "src" / "core" / "enhanced_error_handling.py"
            ).exists(),
            "context_processor": (
                self.project_root / "src" / "core" / "context_aware_processor.py"
            ).exists(),
            "performance_monitor": (
                self.project_root / "tools" / "performance_monitor.py"
            ).exists(),
        }
    ...
    def get_python_path(self) -> str:
        """Get the correct Python path for MCP server (fixes spawn python ENOENT)"""
    ...
    def setup_claude_desktop_config(self) -> bool:
        """Setup optimized Claude Desktop configuration with bug fixes"""
        try:
            # Get correct Python path (fixes spawn python ENOENT error)
                ...
                with open(token_file, "r") as f:
                    for line in f:
                        if "=" in line and not line.strip().startswith("#"):
                            ...
                            token_config[key] = value.strip("\"'")

            # Create optimized Claude Desktop config with fixes
            config = {
                "mcpServers": {
                    "moodleclaude-stable": {
                        ...
                        "env": {
                            "MOODLE_URL": token_config.get(
                                "MOODLE_URL", "http://localhost:8080"
                            ),
                            "MOODLE_TOKEN_BASIC": token_config.get(
                                "MOODLE_BASIC_TOKEN", ""
                            ),
                            "MOODLE_TOKEN_ENHANCED": token_config.get(
                                "MOODLE_BASIC_TOKEN", ""
                            ),  # Use same token with permissions
                            "MOODLE_USERNAME": token_config.get(
        ...
        try:
            ...
            try:
                ...
                processor.add_conversation_turn(
                    "test_session", "Create a Python course", "success"
                )
                suggestions = processor.get_contextual_suggestions("test_session")

                results["functionality_tests"]["context_processing"] = (
    ...
    def generate_setup_report(
        self, validation_results: Dict, performance_results: Dict
    ) -> str:
...
"""
        ...
        for feature, status in validation_results.get(
            "performance_features", {}
        ).items():
...
enhanced error handling, and intelligent context-aware processing.
    ...
    async def run_full_setup(self, update_claude_config: bool = True) -> Dict[str, Any]:
        ...
        setup_results["setup_successful"] = (
            prereq_passed
            and validation_score >= 75
            and (
                setup_results["claude_config"] is True
                or setup_results["claude_config"] == "skipped"
            )
        )

        # Generate report
        report = self.generate_setup_report(
            setup_results["validation"], setup_results["performance_tests"]
        )
        ...
        if setup_results["setup_successful"]:
            ...
            logger.warning(
                "⚠️ Setup completed with issues. Check the report for details."
            )
...
async def main():
    ...
    parser.add_argument(
        "--validate-only", action="store_true", help="Only run validation, skip setup"
    )
    parser.add_argument(
    ...
    if args.validate_only:
        ...
        if results["setup_successful"]:
            ...
            print(
                "3. Monitor performance with: python tools/performance_monitor.py --metrics"
            )
        else:
