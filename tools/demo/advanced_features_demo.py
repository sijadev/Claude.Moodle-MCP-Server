...
class AdvancedFeaturesDemo:
    ...
    async def demo_content_analysis(self):
        ...
        for content_type, content in self.sample_contents.items():
            ...
            print(f"  ⚡ Strategy: {analysis['recommended_strategy'].value.replace('_', ' ').title()}")
    ...
    async def demo_intelligent_session_creation(self):
        ...
        for content_type, content in self.sample_contents.items():
            ...
            result = await self.session_manager.create_intelligent_course_session(
                content=content,
                course_name=f"Demo {content_type.title()} Course"
            )
            ...
            if result['success']:
                ...
                if result.get('immediate_completion'):
                    ...
                    if result.get('processing_plan'):
                        ...
                        print(f"     - Strategy: {plan['strategy'].replace('_', ' ').title()}")
        ...
        if result['success']:
            ...
            if continue_result['success']:
                ...
                if continue_result.get('progress'):
                    ...
                    print(f"   🔢 Parts: {progress['completed_chunks']}/{progress['total_chunks']}")
            else:
                print(f"❌ Continuation failed: {continue_result.get('error', 'Unknown error')}")
    ...
    def demo_adaptive_configuration(self):
        ...
        print(f"  💾 Character limit: {summary['processing_limits']['max_char_length']:,}")
        print(f"  📚 Max sections: {summary['processing_limits']['max_sections']}")
        print(f"  🎯 Adaptation sensitivity: {summary['processing_limits']['adaptation_sensitivity']}")
        ...
        print(f"\n📊 Adaptation Stats:")
        print(f"  🔄 Total adaptations: {summary['adaptation_stats']['total_adaptations']}")
    ...
    def demo_session_analytics(self):
        ...
        if analytics.get('overall'):
            ...
            print(f"  📊 Avg completion rate: {overall.get('avg_completion_rate', 0):.1%}")
            print(f"  📏 Avg content size: {overall.get('avg_content_size', 0):.0f} chars")
        ...
        if analytics.get('processor_metrics'):
            ...
            print(f"  🧠 Learning status: {metrics.get('learning_status', 'Unknown').title()}")
                ...
                if choice == 'q' or choice == 'quit':
                    ...
                    if content:
                        analysis = asyncio.run(
                            self.session_manager.content_processor.analyze_content_complexity(content)
                        )
                    ...
                    if content:
                        result = asyncio.run(
                            self.session_manager.create_intelligent_course_session(content, name)
                        )
                    ...
                    summary = self.config.get_configuration_summary()
                    print(f"Configuration: {json.dumps(summary, indent=2, default=str)}")
                else:
