...
async def test_enrollment_fix():
    ...
    async with MoodleClient(os.getenv('MOODLE_URL'), os.getenv('MOODLE_TOKEN')) as client:
        try:
            ...
            test_course_name = f"Enrollment Test Course - {datetime.now().strftime('%H%M%S')}"
            ...
            if is_enrolled:
                ...
                enrolled_course = next((c for c in user_courses if c.get('id') == course_id), None)
                if enrolled_course:
                    print(f"   Course name: {enrolled_course.get('fullname')}")
                    print(f"   Course URL: {os.getenv('MOODLE_URL')}/course/view.php?id={course_id}")
            
            # Test 4: Check enrollment methods for this course
            print(f"\n🔍 Checking enrollment methods for course {course_id}...")
            try:
                enrol_methods = await client._call_api('core_enrol_get_course_enrolment_methods', {
                    'courseid': course_id
                })
            ...
            print(f"   🔗 Direct access: {os.getenv('MOODLE_URL')}/course/view.php?id={course_id}")
            
            if is_enrolled:
                print(f"\n🎉 SUCCESS: The enrollment fix is working!")
                print(f"   Courses created via Claude Desktop should now appear in 'My Courses'")
            else:
