...
async def test_moodle_connectivity():
    ...
    results = {"overall_status": "unknown", "tests": {}}
            ...
            async with session.get(api_url, params=params, timeout=10) as response:
                if response.status == 200:
                    ...
                    if isinstance(data, list):
                        ...
                        if data:
                            print(
                                f"- Default category: {data[0].get('name', 'Unknown')}"
                            )
            ...
            try:
                ...
                async with session.get(api_url, params=params, timeout=10) as response:
                    if response.status == 200:
                        ...
                        print(
                            f"- ❌ Plugin token request failed: HTTP {response.status}"
                        )
                        results["tests"][
                            "plugin_token"
                        ] = f"❌ Fail (HTTP {response.status})"
    ...
    # Overall assessment
    passed_tests = sum(
        1 for result in results["tests"].values() if result.startswith("✅")
    )
