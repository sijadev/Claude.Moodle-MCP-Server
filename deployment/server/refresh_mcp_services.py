...
async def refresh_service_status():
    ...
    try:
        from src.core.interfaces import IAnalyticsService, ICourseCreationService
        from src.core.service_configuration import (
            PRODUCTION_CONFIG,
            create_configured_container,
            get_service_health_check,
        )
        ...
        if analytics_service:
            try:
                ...
                print(
                    f"- System Health Status: {system_health.get('status', 'unknown')}"
                )
                print(f"- Active Sessions: {system_health.get('active_sessions', 0)}")
                print(
                    f"- Database Accessible: {system_health.get('database_accessible', False)}"
                )
            except Exception as e:
        ...
        all_healthy = (
            ...
            and course_service is not None
