...
class StrategyConfig:
    ...
    strategy_success_rates: Dict[str, float] = field(default_factory=lambda: {
        'single_pass': 0.9,
...
class UserExperienceConfig:
    ...
    continuation_messages: List[str] = field(default_factory=lambda: [
...
class AdaptiveConfig:
    ...
    def load_config(self) -> bool:
        ...
        try:
            ...
            if 'user_experience' in config_data:
                self.user_experience = UserExperienceConfig.from_dict(config_data['user_experience'])
    ...
    def save_config(self) -> bool:
        ...
        try:
            ...
            config_data = {
                ...
                'adaptation_history': self.adaptation_history[-100:],  # Keep last 100 entries
    ...
    def adapt_processing_limits(self, success_rate: float, avg_content_size: int, 
        ...
        try:
            ...
            if success_rate > 0.9 and avg_content_size > self.processing.max_char_length:
                # Increase limit if we're succeeding with larger content
                new_limit = min(
                    int(self.processing.max_char_length * (1 + self.processing.adaptation_sensitivity)),
            ...
            if success_rate > 0.85:
                ...
                self.processing.max_sections = min(
                    int(self.processing.max_sections * 1.05),
                    25
                )
                adaptation_made = True
                ...
                self.processing.max_sections = max(
                    int(self.processing.max_sections * 0.95),
                    5
                )
                adaptation_made = True
            ...
            if adaptation_made:
                ...
                logger.info(f"Processing limits adapted: char_limit={self.processing.max_char_length}, "
    ...
    def adapt_strategy_effectiveness(self, strategy: str, success: bool) -> bool:
        ...
        try:
            ...
            if abs(new_rate - current_rate) > 0.01:
                ...
                logger.info(f"Strategy effectiveness updated: {strategy} = {new_rate:.3f}")
                return True
    ...
    def get_optimal_strategy_thresholds(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {
                'single_pass': thresholds.get('single_pass', self.strategy.single_pass_complexity_threshold),
                'intelligent_chunk': thresholds.get('intelligent_chunk', self.strategy.intelligent_chunk_complexity_threshold),
                'progressive_build': thresholds.get('progressive_build', self.strategy.progressive_build_complexity_threshold)
            }
