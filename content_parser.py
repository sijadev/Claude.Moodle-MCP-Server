...
class ChatContentParser:
    ...
    def __init__(self):
        ...
        self.language_patterns = {
            'python': [r'def\s+\w+', r'import\s+\w+', r'from\s+\w+\s+import', r'if\s+__name__\s*==', r'class\s+\w+'],
            'javascript': [r'function\s+\w+', r'const\s+\w+', r'let\s+\w+', r'var\s+\w+', r'=>', r'console\.log'],
            'typescript': [r'interface\s+\w+', r'type\s+\w+', r':\s*\w+\[\]', r'async\s+function', r'Promise<'],
            'java': [r'public\s+class', r'private\s+\w+', r'public\s+static\s+void\s+main', r'@\w+', r'System\.out'],
            'cpp': [r'#include\s*<', r'int\s+main\s*\(', r'std::', r'cout\s*<<', r'endl'],
            'c': [r'#include\s*<', r'int\s+main\s*\(', r'printf\s*\(', r'malloc\s*\('],
            'html': [r'<html', r'<div', r'<span', r'<!DOCTYPE', r'<body'],
            'css': [r'\.\w+\s*{', r'#\w+\s*{', r'@media', r'font-family:', r'background-color:'],
        ...
        self.topic_keywords = [
            'explanation', 'tutorial', 'guide', 'overview', 'introduction',
            'concept', 'theory', 'definition', 'principle', 'fundamental',
            'basic', 'advanced', 'example', 'demonstration', 'illustration',
            'summary', 'conclusion', 'key points', 'important', 'note that'
        ]
    ...
    def _extract_code_blocks(self, message: str, current_topic: Optional[str] = None) -> List[ContentItem]:
    ...
    def _extract_inline_code(self, message: str, current_topic: Optional[str] = None) -> List[ContentItem]:
        """Extract significant inline code snippets"""
        inline_items = []
    ...
    def _extract_java_title(self, code: str) -> str:
        ...
        method_match = re.search(r'(?:public|private|protected)?\s*(?:static\s+)?[A-Za-z<>\[\]]+\s+(\w+)\s*\(', code)
        if method_match:
        ...
        for paragraph in paragraphs:
            ...
            if self._is_topic_description(paragraph):
                ...
                topic_item = ContentItem(
                    ...
                    title=title,
                    content=paragraph,
                    description=f"Educational content about {topic}" if topic else "Topic description",
                    language=None,
                    topic=topic
                )
    ...
    def _is_topic_description(self, text: str) -> bool:
        ...
        explanatory_phrases = [
            'this means', 'in other words', 'for example', 'that is',
            'it works by', 'the idea is', 'the concept', 'basically',
            'essentially', 'fundamentally', 'the purpose', 'the goal'
        ]
    ...
    def _infer_topic_from_content(self, content: str) -> Optional[str]:
        ...
        tech_topics = {
            ...
            'algorithms': ['algorithm', 'sorting', 'searching', 'complexity', 'data structure'],
        }
        ...
        for item in items:
            if item.type == "code":
                ...
                for existing_item in unique_items:
                    if (existing_item.type == "topic" and 
                        self._are_similar_topics(item.content, existing_item.content)):
                        is_duplicate = True
                        break
